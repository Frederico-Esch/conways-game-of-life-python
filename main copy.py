...
grid = [[(i*size,j*size, (255,255,255)) for i in range(amount)] for j in range(amount)]
...
def mouse_click() -> None:
    ...
    if(last_mouse != (x,y)):
        ...
        _x,_y,c = grid[x][y]
        if c == (255, 255, 255):
            grid[x][y] = (_x,_y,ACTIVE_COLOUR)
        else:
            grid[x][y] = (_x,_y,(255,255,255))
        last_mouse = (x,y)
...
while True:
    ...
    for rows in grid:
        for x,y,colour in rows:
            #x, y, colour = rect
            pg.draw.rect(screen, colour, (x, y, size-2, size-2))
            ...
            if iterate:
                ...
                for neigh in neighbours_loc:
                    ...
                    if nx >= 0 and nx < amount and ny >= 0 and ny < amount:
                        if grid[ny][nx][2] == ACTIVE_COLOUR:
                            n_neighbours+=1
                if n_neighbours == 3:
                    grid[_y][_x] = (x,y,ACTIVE_COLOUR)
                elif n_neighbours < 2 or n_neighbours > 3:
                    grid[_y][_x] = (x,y,(255,255,255))
    pg.display.update()